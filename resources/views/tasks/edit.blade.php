...
@section('content')
    ...
    <div class="w-full max-w-[700px] mx-auto">
        ...
        <div class="mb-stack-lg text-center">
            <h2 class="font-headline-lg text-headline-lg text-on-surface">Edit Task</h2>
            ...
            <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-stack-lg">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Add some details or notes..."
                        class="w-full rounded-lg border border-outline-variant focus:border-primary-container focus:ring-1 focus:ring-primary-container p-3 font-body-md text-body-md bg-white transition-all outline-none resize-none">{{ old('description', $task->description) }}</textarea>
                </div>
                        ...
                        <div class="flex p-1 bg-surface-container rounded-lg gap-1">
                            @foreach (['low' => 'text-secondary', 'medium' => 'text-tertiary', 'high' => 'text-error'] as $level => $color)
                                <button type="button" id="btn-{{ $level }}"
                                    ...
                                    {{ old('priority', $task->priority ?? 'low') === $level ? 'bg-white shadow-sm font-bold ' . $color : 'text-on-surface-variant hover:bg-surface-container-high' }}">
                                    {{ ucfirst($level) }}
                                </button>
                            @endforeach
                        </div>
