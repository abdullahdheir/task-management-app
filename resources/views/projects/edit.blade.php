...
@section('content')
    ...
    <div class="mb-10">
        ...
        <div class="flex justify-between items-end">
            <div>
                <h2 class="font-headline-lg text-headline-lg text-on-surface">{{ $project->name }}</h2>
                <p class="text-body-md text-on-surface-variant mt-1">Managed by {{ $project->owner->name ?? 'Unknown' }} •
            ...
            <section class="bg-surface-container-lowest rounded-xl border border-outline-variant p-stack-lg shadow-sm">
                ...
                <div class="mt-8">
                    <div class="flex justify-between mb-2">
                        ...
                        <span
                            class="font-label-md text-label-md text-secondary font-bold">{{ $project->progress ?? 0 }}%</span>
                    </div>
                    <div class="w-full bg-surface-container-highest h-1.5 rounded-full overflow-hidden">
                        <div class="bg-secondary h-full" style="width: {{ $project->progress ?? 0 }}%"></div>
                    </div>
                </div>
            </section>
            <div class="flex justify-end gap-3">
                <a href="{{ route('projects.show', $project) }}"
                    class="px-6 py-2.5 rounded-lg font-label-md text-label-md text-on-surface-variant hover:bg-surface-container-low transition-colors">Cancel</a>
                <button type="submit"
                    class="bg-primary text-on-primary px-8 py-2.5 rounded-lg font-label-md text-label-md shadow-md hover:shadow-lg transition-all active:scale-[0.98]">Save
                    Changes</button>
            </div>
            </form>
        </div>
